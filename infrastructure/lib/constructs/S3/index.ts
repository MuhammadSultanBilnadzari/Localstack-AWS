...
// 💡 Komentari import config (karena file-nya tidak ada)
...
export class S3 extends Construct {
  ...
  constructor(scope: Construct, id: string, props: Props) {
    ...
    // 💡 Gantikan config.* dengan hard-coded value sementara / dummy
    const configDummy = {
      frontend_subdomain: 'frontend',
      frontend_dev_subdomain: 'dev-frontend',
      domain_name: 'example.com',
    };

    const frontEndSubDomain =
      process.env.NODE_ENV === 'Production'
        ? configDummy.frontend_subdomain
        : configDummy.frontend_dev_subdomain;
    ...
    this.distribution = new Distribution(
      ...
      `Frontend-Distribution-${process.env.NODE_ENV || ''}`,
      {
        ...
        domainNames: [`${frontEndSubDomain}.${configDummy.domain_name}`],
    ...
    new ARecord(scope, `FrontendAliasRecord-${process.env.NODE_ENV || ''}`, {
      ...
      recordName: `${frontEndSubDomain}.${configDummy.domain_name}`,
    });
